...
public class ItemMixingFilter extends APPItem {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4)
    {
		...
		list.add((stack.getTagCompound().getFloat("DurationDegradeFactor") > 0.6f ? EnumChatFormatting.DARK_RED : 
			stack.getTagCompound().getFloat("DurationDegradeFactor") > 0.3f ? EnumChatFormatting.GOLD : EnumChatFormatting.DARK_GREEN) +
					"Time degrade factor: " + ((int)(stack.getTagCompound().getFloat("DurationDegradeFactor")*100))+"%");
		
		list.add((stack.getTagCompound().getFloat("LevelDegradeFactor") > 0.6f ? EnumChatFormatting.DARK_RED : 
			stack.getTagCompound().getFloat("LevelDegradeFactor") > 0.3f ? EnumChatFormatting.GOLD : EnumChatFormatting.DARK_GREEN) +
				"Level degrade factor: " + ((int)(stack.getTagCompound().getFloat("LevelDegradeFactor")*100))+"%" + EnumChatFormatting.RESET);		
		if (stack.getTagCompound().getBoolean("GoodBoost"))
			list.add("Buff Booster"); else
		if (stack.getTagCompound().getBoolean("BadBoost"))
			list.add("Debuff Booster"); else
		if (stack.getTagCompound().getBoolean("SplashBoost"))
	...
	public static void generateCustomInfo(ItemStack stack) {
		if (hasCustomInfo(stack)) return;	//already generated
		else {
			...
			if (random.nextFloat() >= 0.65) {
				...
				if (boost >= 0.8) tags.setBoolean("SplashBoost", true);
				else if (boost >= 0.4) tags.setBoolean("GoodBoost", true);
				else tags.setBoolean("BadBoost", true);
			}
    ...
    {
       if(isRevealed(stack)) return false;
       else { reveal(stack);  return true; }
    }
