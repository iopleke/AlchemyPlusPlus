...
|| (id == Item.netherStar.itemID))
	...
	public static void performExtraction(TileEntityExtractor te) {
		...
		if (te.getStackInSlot(0).itemID == ItemRegistry.appItemSquidEye.itemID) {
			...
			if (te.getStackInSlot(1).getItemDamage() == 8230 || te.getStackInSlot(1).getItemDamage() == 16422) {
			addEffect(te.getStackInSlot(1), new PotionEffect(15, 100));
			}
		} else
		if (te.getStackInSlot(0).itemID == ItemRegistry.appItemConfusion.itemID) {
			//Slowness I - 8234 || splash - 16426
			if (te.getStackInSlot(1).getItemDamage() == 8234 || te.getStackInSlot(1).getItemDamage() == 16426) {
			addEffect(te.getStackInSlot(1), new PotionEffect(9, 100));
			}
		} else
		if (te.getStackInSlot(0).itemID == ItemRegistry.appItemSpringyCord.itemID) {
			//Speed I - 8194 || splash - 16386
			if (te.getStackInSlot(1).getItemDamage() == 8194 || te.getStackInSlot(1).getItemDamage() == 16386) {
			addEffect(te.getStackInSlot(1), new PotionEffect(8, 100));
			}
		} else
		if (te.getStackInSlot(0).itemID == ItemRegistry.appItemIronPowder.itemID) {
			//Strength I - 8201 || splash - 16393
			if (te.getStackInSlot(1).getItemDamage() == 8201 || te.getStackInSlot(1).getItemDamage() == 16393) {
			addEffect(te.getStackInSlot(1), new PotionEffect(11, 100));
			}
		} else
			if (te.getStackInSlot(0).itemID == ItemRegistry.appItemFesteringFlesh.itemID) {
			//Poison I - 8196 || splash - 16388
			if (te.getStackInSlot(1).getItemDamage() == 8196 || te.getStackInSlot(1).getItemDamage() == 16388) {
			...
			if (te.getStackInSlot(1).getItemDamage() == 8193 || te.getStackInSlot(1).getItemDamage() == 16385) {
			...
			if (te.getStackInSlot(1).getItemDamage() == 8268 || te.getStackInSlot(1).getItemDamage() == 16460) {
...
if ((stack1 == null || stack2 == null || filter == null || result1 == null || result2 == null)
		||
	(stack1.itemID != Item.potion.itemID || stack2.itemID != Item.potion.itemID || !(filter.getItem() instanceof ItemMixingFilter))
		||
	(ItemPotion.isSplash(stack1.getItemDamage()) != ItemPotion.isSplash(stack2.getItemDamage()))
		||
	(filter.getMaxDamage() - filter.getItemDamage() < mixingCost(stack1, stack2))
		||
	...
	static public void mix(TileEntityLiquidMixer entity) {
		...
		int newDamage = (int) (entity.getStackInSlot(2).getItemDamage() + 
				((entity.getStackInSlot(2).getTagCompound().getBoolean("Unbreaking")) ? 0.7f : 1f) * mixingCost(entity.getStackInSlot(0), entity.getStackInSlot(1)));
		
		int potionID = entity.getStackInSlot(0).getItemDamage();
		
		List l1 = ((ItemPotion)(entity.getStackInSlot(0).getItem())).getEffects(entity.getStackInSlot(0));
		List l2 = ((ItemPotion)(entity.getStackInSlot(1).getItem())).getEffects(entity.getStackInSlot(1));
		
		entity.setInventorySlotContents(0, entity.getStackInSlot(3));
		entity.setInventorySlotContents(1, entity.getStackInSlot(4));
		
		entity.setInventorySlotContents(3, new ItemStack(Item.potion.itemID, 1, potionID));
		entity.setInventorySlotContents(4, new ItemStack(Item.potion.itemID, 1, potionID));
		
		entity.getStackInSlot(3).setItemName(EnumChatFormatting.RESET + "Potion");
		entity.getStackInSlot(4).setItemName(EnumChatFormatting.RESET + "Potion");
		
		ArrayList compiled = new ArrayList();
		for (int i = 0; i < l1.size(); i++) compiled.add(l1.get(i));
		for (int i = 0; i < l2.size(); i++) compiled.add(l2.get(i));
		...
		for (int i = 0; i < compiled.size(); i++) {
			...
			for (int j = 0; j < compiled.size(); j++) {
				if (i == j) continue;
				if (((PotionEffect) compiled.get(i)).getPotionID() == ((PotionEffect) compiled.get(j)).getPotionID()) 
				{ 
		...
		for (int i = 0; i < compiled.size(); i++) {
		addCustomPotionEffect(entity.getStackInSlot(3), ((PotionEffect) compiled.get(i)), true, entity.getStackInSlot(2));
		addCustomPotionEffect(entity.getStackInSlot(4), ((PotionEffect) compiled.get(i)), true, entity.getStackInSlot(2));
		}
		
		if (entity.getStackInSlot(3).getTagCompound().getTagList("CustomPotionEffects").tagCount() == 0)
		{
			entity.getStackInSlot(3).setItemDamage(0);
			entity.getStackInSlot(3).setItemName(EnumChatFormatting.RESET + "Unleavened Brew");
		}
		
		if (entity.getStackInSlot(4).getTagCompound().getTagList("CustomPotionEffects").tagCount() == 0)
		{
			entity.getStackInSlot(4).setItemDamage(0);
			entity.getStackInSlot(4).setItemName(EnumChatFormatting.RESET + "Unleavened Brew");
		}
			
		entity.getStackInSlot(2).setItemDamage(newDamage);
		if (entity.getStackInSlot(2).getItemDamage() > entity.getStackInSlot(2).getMaxDamage() - 100) entity.setInventorySlotContents(2, null);
		
	}
	
	public static void betterEffect(ArrayList effects, int p1, int p2) {
			if (((PotionEffect)effects.get(p2)).getAmplifier() > ((PotionEffect)effects.get(p1)).getAmplifier())
				effects.remove(p1);
				else if (((PotionEffect) effects.get(p2)).getAmplifier() == ((PotionEffect) effects.get(p1)).getAmplifier()) {
					if (((PotionEffect) effects.get(p2)).getDuration() > ((PotionEffect) effects.get(p1)).getDuration())
					effects.remove(p1);
					else effects.remove(p2);
				} else effects.remove(p2);
	}
	
	public static void addCustomPotionEffect(ItemStack stack, PotionEffect e, boolean degrade, ItemStack filter) {
		Random random = new Random();
		boolean badBonus = (isBadEffect(e.getPotionID()) && filter.getTagCompound().getBoolean("BadBoost"));
		boolean goodBonus = (!(isBadEffect(e.getPotionID())) && filter.getTagCompound().getBoolean("GoodBoost"));
		...
		if (stack.hasTagCompound())	{
			...
			NBTTagList list = stack.getTagCompound().getTagList("CustomPotionEffects");
			...
			else e.writeCustomPotionEffectToNBT(effectTag);
	...
	public static NBTTagCompound degradeAndWriteEffect(NBTTagCompound tag, PotionEffect e, ItemStack filter) {
		...
		if (random.nextFloat() < ItemMixingFilter.getLevelDegradeFactor(filter)*lvlDegradeChance)
		{
			if (amp == 0) return null;
			else tag.setByte("Amplifier", (byte)(e.getAmplifier() - 1));
		...
		dur -= (ItemMixingFilter.getTimeDegradeFactor(filter)*dur*random.nextFloat());
	...
	} else tag.setByte("Amplifier", (byte)(e.getAmplifier()));
	
	int dur = e.getDuration() + ((int)(e.getDuration()*0.2f*random.nextFloat()));
	tag.setInteger("Duration", dur);
	...
	static boolean isBadEffect(int id) {
		if (id == 1 || id == 3 || id == 5 || id == 6 || id == 8 || id == 10 || id == 11 || id == 12 || id == 13 || id == 14 || id == 16) return false;
		else return true;
	}
	
	public static void sprayingDegrade(ItemStack potion) {
		ArrayList<PotionEffect> list = (ArrayList<PotionEffect>) ((ItemPotion)(potion.getItem())).getEffects(potion);
		int effectAmount = 0;
		
		NBTTagList effects = new NBTTagList();
		if (!potion.hasTagCompound()) potion.setTagCompound(new NBTTagCompound());
